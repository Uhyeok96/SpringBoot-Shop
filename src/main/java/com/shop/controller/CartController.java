...
public class CartController {
    ...
    public @ResponseBody ResponseEntity order(@RequestBody @Valid CartItemDto cartItemDto, BindingResult bindingResult, Principal principal){
        ...
        try {
            cartItemId = cartService.addCart(cartItemDto, email);  // 화면으로부터 넘어온 장바구니에 담을 상품 정보와 로그인한 회원의 이메일 정보를 이용하여 장바구니에 상품을 담음.
        } catch(Exception e){
    ...
    // 장바구니 상품의 수량 변경 업데이트
    @PatchMapping(value = "/cartItem/{cartItemId}")  // 353 추가 (일부만 업데이트할 때 PATCH 사용함)
